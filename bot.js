...
${chalk.blue.italic('? Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'Abuser' || config.AFPLK == 'ABU' || config.AFPLK == 'Abu' || config.AFPLK == 'pinky') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'Abuser' || config.AFPLK !== 'ABU' || config.AFPLK !== 'Abu' || config.AFPLK !== 'pinky') {
         console.log(
        ...
        console.log(
            chalk.green.bold('?Abu Ser Working ?' + config.WORKTYPE + ''));
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 9 && getGMTm == 01) {
            ...
            return await conn.sendMessage(conn.user.jid, '*[ DAILY ANNOUNCEMENTS BY ABU ]*\n\n' + announce, MessageType.text);
        }
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ?ABU SER? ~______~*' +
                                    '\n*EVADAYO CHERIYA EROR UND' +
                                    '\n\n*ABU SERINTE PM MESSEG AYCHAL MATHI:'wa.me/917025994178  ' + error + '*\n'
                                    , MessageType.text);
                            }
                        }
                    }
